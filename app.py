...
import io
import os   # <-- added for file-based persistence
...
def get_initial_mock_data():
    """Provides a fresh copy of the mock database for first-time initialization."""
    return {
        "accounts": {
...
def save_data(data):
    """Save all wallet data into a JSON file (simple persistence)."""
    try:
        with open(DB_FILE, "w", encoding="utf-8") as f:
            json.dump(data, f)
    except Exception:
        # For safety: ignore write errors on managed platforms
        pass

def load_data():
    """Load wallet data from JSON; if missing/corrupt, start with mock data."""
    if os.path.exists(DB_FILE):
        try:
            with open(DB_FILE, "r", encoding="utf-8") as f:
                return json.load(f)
        except Exception:
            # If file is corrupted, reset to initial data
            data = get_initial_mock_data()
            save_data(data)
            return data
    else:
        data = get_initial_mock_data()
...
# 👉 load from JSON instead of using fresh mock each time
...
def format_currency_with_color(amount, transaction_type):
    ...
    if transaction_type.endswith("_in") or transaction_type == "deposit":
        return f"<span style='color:#28a745; font-weight:500;'>+ ₹{amount:,.2f}</span>"
    elif transaction_type.endswith("_out") or transaction_type == "withdraw":
        return f"<span style='color:#dc3545; font-weight:500;'>- ₹{amount:,.2f}</span>"
    return f"₹{amount:,.2f}"
...
def display_high_value_warning(amount):
    ...
    if amount >= HIGH_VALUE_TRANSACTION_THRESHOLD:
        st.warning(
            f"⚠️ **High-Value Transaction:** You are initiating a transaction for ₹{amount:,.2f}. Please verify all details before proceeding.",
            icon="❗",
        )
    ...
    with col_form:
        ...
        with st.form("login_form"):
            ...
            if st.form_submit_button("Login", use_container_width=True, type="primary"):
                handle_login_attempt(username, pin)
...
def render_create_account_page():
    ...
    inject_custom_css_black_theme()
    st.markdown("<div class='login-container'>", unsafe_allow_html=True)
    st.title("Create Your Gemini Wallet")
    st.markdown("Join the most intuitive digital payment platform.")
    with st.form("create_account_form"):
        ...
        if st.form_submit_button("Create My Account", use_container_width=True, type="primary"):
            handle_account_creation(username, display_name, pin1, pin2, initial_amount)
...
def render_main_app():
    ...
    with st.sidebar:
        render_sidebar_navigation(user, user_data)
...
def render_dashboard_content(user, user_data):
    ...
    st.title("🏠 Dashboard")
    st.markdown("An overview of your account activity and pending actions.")
    ...
    if not pending_requests:
        st.info("You have no pending payment requests. All clear!", icon="✅")
    else:
        for req in pending_requests:
            with st.container(border=True):
                ...
                with col1:
                    st.write(
                        f"**From:** {st.session_state.data['accounts'][req['requester']]['display_name']} (@{req['requester']})"
                    )
                    st.write(f"**Amount:** ₹{req['amount']:,.2f}")
                    st.caption(f"**Note:** '{req['note']}' | **Received:** {req['timestamp']}")
                with col2:
                    if st.button("Approve & Pay", key=f"pay_{req['id']}", use_container_width=True, type="primary"):
                        handle_payment_request(user, req, "approved")
                    if st.button("Decline", key=f"dec_{req['id']}", use_container_width=True):
                        handle_payment_request(user, req, "declined")

    st.subheader("Recent Activity")
    transactions = user_data["transactions"]
    if not transactions:
        st.info("Your transaction history is empty.", icon="📜")
    else:
        ...
        st.markdown(
            df[["timestamp", "type", "Amount", "counterparty", "note"]].to_html(escape=False, index=False),
            unsafe_allow_html=True,
        )
...
def render_deposit_page(user, user_data):
    ...
    with st.form("deposit_form", clear_on_submit=True):
        ...
        if st.form_submit_button("Deposit Funds", use_container_width=True, type="primary"):
            handle_deposit(user, amount, note)
...
def render_withdraw_page(user, user_data):
    ...
    with st.form("withdraw_form", clear_on_submit=True):
        ...
        if st.form_submit_button("Withdraw Funds", use_container_width=True, type="primary"):
            handle_withdrawal(user, amount, category, pin)
...
def render_transfer_page(user, user_data):
    ...
    with st.form("transfer_form"):
        ...
        receiver_username = st.selectbox(
            "Select Recipient", options=list(receiver_options.keys()), format_func=lambda u: receiver_options[u]
        )
        ...
        if st.form_submit_button("Send Money Securely", use_container_width=True, type="primary"):
            handle_transfer(user, receiver_username, amount, note, category, pin)
...
def render_qr_generate_page(user, user_data):
    ...
    with st.form("qr_generate_form"):
        ...
        if st.form_submit_button("Generate QR Code", use_container_width=True, type="primary"):
            handle_qr_generation(user, amount, note)
...
def render_qr_pay_page(user, user_data):
    ...
    with st.form("qr_pay_form"):
        ...
        if st.form_submit_button("Make Payment", use_container_width=True, type="primary"):
            handle_qr_payment(user, payload_str, category, pin)
...
def render_history_page(user, user_data):
    ...
    transactions = user_data["transactions"]
    if not transactions:
        st.info("No transactions to display.", icon="📜")
    else:
        df = pd.DataFrame(sorted(transactions, key=lambda x: x["timestamp"], reverse=True))
        df["Amount"] = df.apply(lambda row: format_currency_with_color(row["amount"], row["type"]), axis=1)
        st.markdown(
            df[["timestamp", "type", "Amount", "counterparty", "note", "category"]]
            .fillna("-")
...
def render_category_report_page(user, user_data):
    ...
    for txn in user_data["transactions"]:
        if txn["type"] in {"transfer_out", "qr_out", "withdraw"} and txn.get("category"):
            spend[txn["category"]] += txn["amount"]
    if not spend:
        st.info("No categorized spending data available.", icon="💡")
    else:
        spend_df = pd.DataFrame(list(spend.items()), columns=["Category", "Amount (₹)"]).set_index("Category")
        st.bar_chart(spend_df, color="#0d6efd")
        st.table(spend_df.style.format("₹{:,.2f}"))
...
def render_monthly_report_page(user, user_data):
    ...
    for txn in user_data["transactions"]:
        ...
        if txn["type"] in {"deposit", "transfer_in", "qr_in"}:
            summary[month]["Inflow (₹)"] += txn["amount"]
        elif txn["type"] in {"withdraw", "transfer_out", "qr_out"}:
            summary[month]["Outflow (₹)"] += txn["amount"]
    if not summary:
        st.info("No transaction data to summarize.", icon="🗓️")
    else:
...
def render_top_payees_report_page(user, user_data):
    ...
    for txn in user_data["transactions"]:
        if txn["type"] in {"transfer_out", "qr_out"}:
            ...
            payees[payee_name]["Payments"] += 1
            payees[payee_name]["Total Amount (₹)"] += txn["amount"]
    if not payees:
        st.info("You haven't made any payments to other users yet.", icon="👥")
    else:
        df = (
            pd.DataFrame.from_dict(payees, orient="index")
            .sort_values(by="Total Amount (₹)", ascending=False)
        )
        df.index.name = "Payee"
        st.table(df.style.format({"Total Amount (₹)": "₹{:,.2f}", "Payments": "{:}"}))
...
def render_settings_page(user, user_data):
    ...
    with st.form("change_pin_form"):
        ...
        if st.form_submit_button("Update My PIN", use_container_width=True, type="primary"):
            handle_pin_change(user, old_pin, new_pin1, new_pin2)

# ==============================================================================
# S7: BACKEND LOGIC HANDLERS (with persistence)
# ==============================================================================
    ...
    if username in accounts:
        ...
        if time.time() < user_acc.get("locked_until", 0):
            ...
            if user_acc["failed_attempts"] >= MAX_LOGIN_ATTEMPTS:
                ...
                st.error(
                    f"❌ Incorrect PIN. {MAX_LOGIN_ATTEMPTS - user_acc['failed_attempts']} attempts remaining."
                )
            save_data(st.session_state.data)
    else:
...
def handle_account_creation(username, display_name, pin1, pin2, initial_amount):
    ...
    if not username or not display_name:
        ...
        accounts[username] = {
            ...
            "transactions": [
                create_transaction_record("deposit", initial_amount, "Initial deposit", None, "self")
            ],
            "failed_attempts": 0,
            "locked_until": 0,
            "requests": [],
        ...
        save_data(st.session_state.data)
        st.success(f"✅ Welcome, {display_name}! Account created.")
        st.balloons()
        time.sleep(2)
        st.session_state.page = "login"
        st.rerun()
    ...
    if pin == user_data["pin"]:
        if amount > user_data["balance"]:
            ...
            user_data["transactions"].append(
                create_transaction_record("transfer_out", amount, note, category, receiver)
            )
            st.session_state.data["accounts"][receiver]["transactions"].append(
                create_transaction_record("transfer_in", amount, note, None, sender)
            )
            save_data(st.session_state.data)
            st.success(
                f"Sent ₹{amount:,.2f} to {st.session_state.data['accounts'][receiver]['display_name']}!"
            )
            st.balloons()
    else:
...
def handle_qr_payment(payer, payload_str, category, pin):
    ...
    try:
        ...
        if not all([payee, amount > 0]):
            st.error("Invalid QR payload.")
        elif payee == payer:
            st.error("You cannot pay yourself.")
            ...
            user_data["transactions"].append(
                create_transaction_record("qr_out", amount, note, category, payee)
            )
            st.session_state.data["accounts"][payee]["transactions"].append(
                create_transaction_record("qr_in", amount, note, None, payer)
            )
            save_data(st.session_state.data)
            st.success(
                f"Paid ₹{amount:,.2f} to {st.session_state.data['accounts'][payee]['display_name']}!"
            )
            st.balloons()
...
def handle_deposit(user, amount, note):
    ...
    user_data["balance"] += amount
    user_data["transactions"].append(
        create_transaction_record("deposit", amount, note or "Self Deposit", None, "self")
    )
    save_data(st.session_state.data)
...
def handle_withdrawal(user, amount, category, pin):
    ...
    if pin == user_data["pin"]:
        if amount > user_data["balance"]:
            ...
            user_data["balance"] -= amount
            user_data["transactions"].append(
                create_transaction_record("withdraw", amount, "Cash withdrawal", category, "cash")
            )
            save_data(st.session_state.data)
...
def handle_pin_change(user, old_pin, new_pin1, new_pin2):
    ...
    if old_pin != user_data["pin"]:
        ...
        user_data["pin"] = new_pin1
        save_data(st.session_state.data)
        st.success("✅ PIN changed successfully!")

def handle_payment_request(payer_username, request, new_status):
    payer_data = st.session_state.data["accounts"][payer_username]
    req_username = request["requester"]
    requester_data = st.session_state.data["accounts"][req_username]
    if new_status == "approved":
        amount = request["amount"]
        if payer_data["balance"] >= amount:
            payer_data["balance"] -= amount
            requester_data["balance"] += amount
            note = f"Payment for: {request['note']}"
            payer_data["transactions"].append(
                create_transaction_record("transfer_out", amount, note, "Bills", req_username)
            )
            requester_data["transactions"].append(
                create_transaction_record("transfer_in", amount, note, None, payer_username)
            )
